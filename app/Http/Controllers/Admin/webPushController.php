...
class webPushController extends Controller
{
	...
	{
		$settingInfo = Settings::where("keyname", "setting")->first();
        $webPushLists = PushDevices::orderByDesc('id')->paginate($settingInfo->item_per_page_back);
	...
	public function saveWebPush(Request $request)
	{
		...
		try {
			...
			$WebPushs->message_for = $request->input('deviceOf');
			$WebPushs->save();
			//send message
	...
	public static function saveToken(Request $request)
	{
		if (!empty($request->token)) {
			$webPush = PushDevices::where("token", $request->token)->first();
			if (empty($webPush->id)) {
				$webpushnew = new PushDevices;
				$webpushnew->token = $request->token;
				$webpushnew->save();
			}
		}
	}
	...
	{
		$webpushLists = PushDevices::where('id', '!=', '0')->where('device', '=', 'web')->get();
	...
	public function pushPorcessing($deviceOf , $title, $message)
	{
		$data = array(
		...
		$webpushLists = PushDevices::where('device', '!=', 'web')->where('device', '!=', 'admin')
            ->when($deviceOf == "User" , function ($sq) {
			    ...
			    $sq->where('user_type', 'App\Freelancer');
		    })->get();
	...
	{
		$token = [];
		$webpushLists = PushDevices::where('user_id', '=', $customerid)->where('device', '=', 'android')->orwhere('device', '=', 'ios')->get();
		if (!empty($webpushLists) && count($webpushLists) > 0) {
