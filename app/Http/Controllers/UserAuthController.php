...
class UserAuthController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('website.auth.login', ['settings' => $settings]);
    }
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::guard('user')->attempt($credentials, $remember))
        {
            ...
            //save logs
            $key_name   = "login";
            $key_id     = Auth::guard('user')->user()->id;
            $message    = Auth::guard('user')->user()->name . " is logged in to User Panel.";
    ...
    public function logout(Request $request)
    {
        ...
        $key_name   = "logout";
        $key_id     = Auth::guard('user')->user()->id;
        $message    = Auth::guard('user')->user()->name . " is logged out from User Panel.";
        $created_by = Auth::guard('user')->user()->id;
        Common::saveLogs($key_name,$key_id,$message,$created_by);
        //end save logs
    ...
    {
        $settings = Settings::where("keyname", "setting")->first();
        return view('website.auth.forgot', ['settings' => $settings]);
    }
    ...
    public function sendResetLinkEmail(Request $request)
    {
        ...
        if (empty($user->id)) {
            ...
            $appendMessage = "<b><a href='" . url('user/forgot/' . $token) . "'>" . trans('adminMessage.passwordresetlink') . "</b>";
            $data = [
                'dear' => trans('adminMessage.dear') . ' ' . $user->name,
            ...
            $key_name = "forgot";
            $key_id = $user->id;
            $message = $user->name . " has requested a forgot password link.";
            $created_by = $user->id;
            Common::saveLogs($key_name, $key_id, $message, $created_by);
            //end save logs
    ...
    public function resetPassword(Request $request, $token)
    {
        ...
        $admin = User::where("email", $request->email)->where("password_token", $token)->first();

        if (empty($user->id)) {
            return redirect('/user/forgot/' . $token)
            ...
            $appendMessage = "";
            $appendMessage .= "<b>" . trans('adminMessage.username') . " : </b>" . $user->username;
