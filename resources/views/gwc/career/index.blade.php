...
{{--                    <td>{{ $resume->mobile }}</td>--}}
                                     ...
                                     <ul class="kt-nav">
                                         @if(auth()->guard('admin')->user()->can($data['viewPermission']))
                                             <li class="kt-nav__item">
                                                 <a href="{{url('gwc/career/'.$resume->id.'/view')}}" class="kt-nav__link">
                                         ...
                                         @if(auth()->guard('admin')->user()->can($data['deletePermission']))
                                             <li class="kt-nav__item">
                                                 <a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$resume->id}}" class="kt-nav__link">
            ...
            @endforeach
            <tr>
                <td colspan="8" class="text-center">{{ $resumes->links() }}</td>
            </tr>
        @else
            <tr>
