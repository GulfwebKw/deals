...
@section('viewContent')
                                    ...
                                    <div class="d-flex flex-stack py-4 userBox">
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="symbol symbol-45px symbol-circle">
                                                ...
                                                <img alt="Pic" src="{{$resource['resources']->last()->user->image}}">
                                            </div>
                                            <!--end::Avatar-->
                                            <!--begin::Details-->
                                            <div class="ms-5">
                                                <a href="#" onclick="ajaxRunUrlHtml('{{url($data['url'] . 'user/' . $resource['resources'][0]->user_id . '/messages')}}')" class="fs-5 fw-bolder text-gray-900 text-hover-primary mb-2 userBoxName">{!! $resource['resources'][0]['user']->first_name . ' '. $resource['resources'][0]['user']->last_name !!}</a>
    ...
    <script>
        ...
        @if(count($resources) and ! request()->has('user_id'))
        @foreach($resources as $key=>$resource)
        document.addEventListener("DOMContentLoaded", function(){
            ajaxRunUrlHtml('{{url($data['url'] . 'user/' . $resource['resources'][0]->user_id . '/messages')}}')
        });
        @break
        @endforeach
        @elseif (request()->has('user_id') )
            document.addEventListener("DOMContentLoaded", function () {
                ajaxRunUrlHtml('{{url($data['url'] . 'user/' . request()->user_id . '/messages')}}')
            });
        @endif
    </script>
@endsection