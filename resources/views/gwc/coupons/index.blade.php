...
@section('indexContent')
    <table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
        ...
        <tr>
            ...
            <th>type</th>
            <th>value</th>
            <th>{{__('adminMessage.date_range')}}</th>
            <th>{{__('adminMessage.price_range')}}</th>
            <th>{{__('adminMessage.usage_limit')}}</th>
            <th>{{__('adminMessage.for')}}</th>
        ...
        @if(count($resources))
            ...
            @foreach($resources as $resource)
                <tr class="search-body">
                    ...
                    <td>
                        {!! $resource->title_en !!} <br> {!! $resource->title_ar !!}
                    </td>
                    <td>
                    ...
                    <td>
                        {!! $resource->coupon_value !!}
                    </td>
                    <td>
                        {!! $resource->date_start !!} <br> {!! $resource->date_end !!}
                    </td>
                    <td>
                        {!! $resource->price_start !!} <br> {!! $resource->price_end !!}
                    </td>
                    <td>
                        {!! $resource->usage_limit !!}
                    </td>
                    <td>
                        {!! $resource->is_for !!}
                    </td>
            ...
            @endforeach
            <tr>
                <td colspan="12" class="text-center">{{ $resources->links() }}</td>
            </tr>
        @else
            <tr>
                <td colspan="12"
