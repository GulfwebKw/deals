...
@method('PUT')
            ...
            <div class="form-group">
                <div class="row">
                    ...
                    <div class="col-lg-12 col-md-12">
                        <div class="form-group">
                            <input id="lat" type="hidden" name="lat" value="{{old('lat',$resource->lat)}}">
                            <input id="lng" type="hidden" name="lng" value="{{old('lng',$resource->lng)}}">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function defaultLatLng() {
            var lat = '{{old('lat',$resource->lat)}}'
            var long = '{{old('lng',$resource->lng)}}'
            document.getElementById('map').innerHTML = ' <label>my map<span>*</span></label>   <div id="mapSender" style="height: 400px"></div>'
            var token = 'pk.eyJ1Ijoic29oZWlsdmFpbyIsImEiOiJja2kxcnUyYTUwNW03MnhudDNsOGRwNG94In0.h3EW-3gLt4EccaIq9tImIw';
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            if ('{{old('lat',$resource->lat)}}' && '{{old('lng',$resource->lng)}}') {
                defaultLatLng();
            } else {
                getLatLong();
            }
